...
#define PRINTK_PREFIX_TS_LEN 22
#define PRINTK_PREFIX_INFO_LEN 44
#endif
...
#ifdef CONFIG_HONOR_PRINTK_PREFIX
static int gen_prefix_ts(char *buf, size_t sz)
{
	struct tm tm_rtc;
	time64_t cur_secs;
	static int len;
	...
	if (time_after(jiffies, prev_jiffies + 1 * HZ)) {
		...
		len = snprintf(buf, sz, "[%lu:%.2d:%.2d %.2d:%.2d:%.2d]",
...
static int gen_prefix_info(char *buf, size_t sz)
{
	int len = 0;
	int this_cpu = smp_processor_id();

	if (in_task())
		len = snprintf(buf, sz, "(%x)[%d:%s]", this_cpu, current->pid, current->comm);
	else
		len = snprintf(buf, sz, "(%x)", this_cpu);
		  ...
		  const char *fmt, va_list args)
{
	...
	char prefix_buf[8];
#ifdef CONFIG_HONOR_PRINTK_PREFIX
	static char prefix_ts[PRINTK_PREFIX_TS_LEN] = {0};
	char prefix_info[PRINTK_PREFIX_INFO_LEN] = {0};
	u16 prefix_ts_len = 0;
	u16 prefix_info_len = 0;
#endif
	u8 *recursion_ptr;
	u16 reserve_size;
	va_list args2;
...
#ifdef CONFIG_HONOR_PRINTK_PREFIX
	prefix_ts_len = gen_prefix_ts(&prefix_ts[0], sizeof(prefix_ts));
	prefix_info_len = gen_prefix_info(&prefix_info[0], sizeof(prefix_info));
	reserve_size += (prefix_ts_len + prefix_info_len);
#endif
	...
	/* fill message */
#ifdef CONFIG_HONOR_PRINTK_PREFIX
	memcpy(&r.text_buf[0], prefix_ts, prefix_ts_len);
	memcpy(&r.text_buf[prefix_ts_len], prefix_info, prefix_info_len);
	text_len = printk_sprint(&r.text_buf[prefix_ts_len + prefix_info_len],
					reserve_size - prefix_ts_len - prefix_info_len,
					facility, &flags, fmt, args);
	text_len += (prefix_ts_len + prefix_info_len);
#else
