...
TRACE_EVENT(sched_switch,
	...
	TP_fast_assign(
		...
		/* XXX SCHED_DEADLINE */
#ifdef CONFIG_HN_SCHED_TRACE_ENHANCEMENT
		kernfs_name(task_get_css(next, cpuset_cgrp_id)->cgroup->kn,
					__entry->cpuset, TASK_COMM_LEN);
		kernfs_name(task_get_css(next, cpu_cgrp_id)->cgroup->kn,
					__entry->cpuctl, TASK_COMM_LEN);
...
next_comm=%s next_pid=%d next_prio=%d cpuset=%s cpuctl=%s task_util=%lu uclamp_task_util=%lu \
cpus=%d active_cpus=%d tags=%llu rsum=%llu",
#else
